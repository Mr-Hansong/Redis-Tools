...
# *Required :
# *   pip install redis argparse commands random progressbar
#
'''
...
DescriptionInfo = "%(prog)s Required install redis,argparse,commands,random,progressbar python modules. If you choose remote-largerkey, also need install redis client and put redis-cli in /usr/bin/"
...
if ( OperType == 'zombiekey' ) : 
	...
	widgets = ['%d keys: ' % TotleKeyCnt, Percentage(), ' ', Bar(marker=RotatingMarker('>-=')),' ', ETA(), ' ', FileTransferSpeed()]
...
# -*- Analysis largekey -*-
	...
	# -*- Start ProgressBar -*-
	widgets = ['%d FileLine: ' % int(LineCnt), Percentage(), ' ', Bar(marker=RotatingMarker('>-=')),' ', ETA(), ' ', FileTransferSpeed()]
